...
class FrontendController extends Controller
{
    ...
    public function getAllBDNews(Request $request){
        ...
        if($upazila_id != null){
            ...
            $active_upazila = Upazila::find($upazila_id);
            return view('_front.pages.country_news', compact('all_bd_news','divisions','division_id','district_id','upazila_id','active_division','active_district','active_upazila'));
        }
...
//    }
    ...
    { 
        ...
        }else if($category_slug == 'privacy-policy'){
            return view('_front.pages.privacyPolicy');
    ...
    {   
        view_counter();
        $post = Post::find($id);
        // $posts = Post::where('post_status',1)->whereDate("created_at",">", Carbon::now()->subMonths(6))->orderBy('view_count','desc')->skip(0)->take(10)->get();
    ...
    {
        $code = $this->settings->live_streaming_code;
        $status = $this->settings->streaming_status;
    ...
    public function auto_load(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            foreach ($posts as $post) {
                $html.='<div class="mt-5"><h1><a href="'.news_url($post).'">'.$post->headline.'</a></h1><p>'.$post->post_content.'</p></div>';
            }
            return $html;
        }
       ...
       foreach($data as $row)
       {
        ...
        $output .= '
         <div class="col-md-3 col-lg-3 col-6">
                ...
                <a class="card-img-top" href="'.news_url($row).'">
                    <div class="media">
    ...
    public function loadmore_category_post(Request $request, $category_id)
    {
     ...
     if($request->ajax())
     {
      ...
      { 
          ...
          if(!empty($settings->lazy_image)){
              $lazy_img = asset($settings->lazy_image);
          }else{
                                        ...
                                        <div class="media-body d-grid">
                                            <ul class="list-inline ">
                                                <li><h4 class="media-heading"><a
                                                                href="'.news_url($row).'"><strong>'.$row->headline.'</strong></a>
                                                    </h4></li>
    ...
    public function singleVideoGallery( $id, $title)
    {
    ...
    public function livestreamShow(){ 
        return view('_front.pages.livestream');
    }
