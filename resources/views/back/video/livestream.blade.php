      ...
      <div class="col-lg-10">
        <div class="card shadow-lg border-0 rounded-lg mt-4 mb-3">
          ...
          <div class="card-body">
              <div class="mb-2">
                    <label class="switch">
                    <input class="changeLivestreamStatus" type="checkbox" data-id="{{$item->id}}" {{ $item->status ? 'checked' : '' }}>
                    <span class="slider round"></span>
                    </label>
              </div>
          
            <form role="form" method="post" action="{{ route('livestream.update',['id'=>$item->id]) }}" enctype="multipart/form-data">
              @csrf
                <div class="form-group body__edit">
                    <label class="mb-1" for="content">Video Stream</label>
                    <script src="https://cdnjs.cloudflare.com/ajax/libs/tinymce/4.9.11/tinymce.min.js"></script>
                    <textarea id="content" name="content" class="my-editor" rows="15">{!! $item->content !!}</textarea>
                    <script>
                        var editor_config = {
                            path_absolute: "/",
                            selector: "#content",
